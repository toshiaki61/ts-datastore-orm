    ...
    getInsertData(entity) {
        ...
        if (this.isEmptyKey(key)) {
            if (isGenerateId) {
                ...
                throw new TsDatastoreOrmError_1.TsDatastoreOrmError(`_id must not be 0, empty string or undefined.`);
            }
        }
        else {
    ...
    validateKey(keys, namespace, kind, checkEmptyKey = true) {
        for (const key of (Array.isArray(keys) ? keys : [keys])) {
            if (key.namespace !== namespace) {
                throw new TsDatastoreOrmError_1.TsDatastoreOrmError(`Namespace not match. Entity namespace is "${key.namespace}". While the expected namespace is "${namespace}".`);
            }
            if (key.kind !== kind) {
                throw new TsDatastoreOrmError_1.TsDatastoreOrmError(`Kind not match. Entity kind is "${key.kind}". While the expected kind is "${kind}".`);
            }
...
exports.tsDatastoreOrm = new TsDatastoreOrm();
